...
@section('content')
    ...
    <h1 class="mb-1">{{ $talk->title }}</h1>

    <div class="text-grey-darker text-lg mb-8">
        @published($talk)
            {{ $talk->published_at->format('d/m/Y h:i') }}
        @else
            Draft
        @endpublished
    </div>
    ...
    <form action="{{ route('dashboard.talks.store') }}" method="POST">
        ...
        <div class="bg-white {{ $errors->count() ? 'border-red' : 'border-blue'}} border-t-4 px-6 py-8 rounded shadow">
            ...
            <input class="border-none bg-grey-lightest block mb-8 px-4 py-3 shadow w-full" type="text" name="title" value="{{ old('title', $talk->title) }}">

            <label class="block font-bold mb-2" for="event">Event</label>
            <input class="border-none bg-grey-lightest block mb-8 px-4 py-3 shadow w-full" type="text" name="event" value="{{ old('event', $talk->event) }}">

            <label class="block font-bold mb-2" for="event">Slide Deck URL</label>
            <input class="border-none bg-grey-lightest block mb-8 px-4 py-3 shadow w-full" type="text" name="slide_deck_url" value="{{ old('slide_deck_url', $talk->slide_deck_url) }}">

            <label class="block font-bold mb-2" for="event">Video URL</label>
            <input class="border-none bg-grey-lightest block mb-8 px-4 py-3 shadow w-full" type="text" name="video_url" value="{{ old('video_url', $talk->video_url) }}">
